...
DCQ.abs <- function(refExpr, geneExpr, marker_set = NULL, number_of_repeats = 10, 
                    alpha = 0.05, lambda = 0.2) 
{
  ...
  if (ncol(geneExpr) == 1) {
    geneExpr <- cbind(geneExpr, geneExpr)
    fixOneCol <- TRUE
  }
  else {
    fixOneCol <- FALSE
  ...
  cellCounts <- try(ComICS::dcq(reference_data = refExpr, mix_data = geneExpr, 
                                marker_set = marker_set, number_of_repeats = number_of_repeats, 
                                lambda_min = lambda, alpha_used = alpha))
  sink()
  if (inherits(cellCounts, "try-error")) {
    return(cellCounts)
  }
  cellCoefVar <- cellCoefVar.m <- t(cellCounts$stdev/cellCounts$average)
  cellCountsPercent <- cellCountsPercent.m <- t(cellCounts$average * 
                                                  100)
